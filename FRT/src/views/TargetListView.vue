...
const fetchBlacklist = async () => {
    ...
    try {
        const response = await axios.get("/api/target/black");
...
const addToBlacklist = async () => {
    ...
    try {
        await axios.post("/api/target/black", { url_path: newUrl.value.trim(), type: "blacklist" });
...
const deleteFromBlacklist = async (id) => {
    try {
        await axios.delete(`/api/target/black/${id}`);
...
const saveEditedBlacklist = async (updatedRecord) => {
	try {
		await axios.put(`/api/target/black/${updatedRecord._id}`, { url_path: updatedRecord.url_path });
